...
class Ctx:
    ...
    started_at: float = field(default_factory=time.time)   # ← 수정
    err: Optional[str] = None

class Updater:
    def __init__(self, storage: Storage, transport: Transport, verifier: Verifier, installer: Installer, reporter):
    ...
    def run(self, req: UpdateRequest):
        ...
        try:
            ...
            if self.state in (State.INSTALL, State.REPORT):
                try:
                    self._go(State.ROLLBACK)
                except Exception as re:
                    self.ctx.err += f" | rollback_err={re}"
            # 에러 리포트 추가
            try:
                if hasattr(self.reporter, "error"):
                    self.reporter.error(self.ctx.err, {"state": self.state.name})
                else:
                    self.reporter.report("error", {"reason": self.ctx.err, "state": self.state.name})
            except Exception:
                pass
    ...
    def _go(self, s: State):
        ...
        if s == State.FETCH_META:
            self.ctx.meta_local = self.transport.fetch(
                self.ctx.req.meta_url,
                self.storage.meta_path(self.ctx.req.version),
            )

        elif s == State.DOWNLOAD:
            self.ctx.image_local = self.transport.fetch(
                self.ctx.req.image_url,
                self.storage.image_path(self.ctx.req.version),
            )
            # 빠른 해시 확인으로 대용량 전송 오류 조기 차단
            ...
            if not vr.ok:
                raise RuntimeError(f"verify failed: {vr.reason}")

        elif s == State.INSTALL:
            ir: InstallResult = self.installer.install(self.ctx.image_local, self.ctx.req.version)
            if not ir.ok:
                raise RuntimeError(f"install failed: {ir.reason}")

        elif s == State.REPORT:
            if hasattr(self.reporter, "ok"):
                self.reporter.ok(self.ctx.req.version)
            else:
                self.reporter.report("success", {"version": self.ctx.req.version})

        elif s == State.ROLLBACK:
            self.installer.rollback()
            if hasattr(self.reporter, "rollback"):
                self.reporter.rollback(self.ctx.req.version)
            else:
    ...
    def _snapshot(self):
        return {
            "version": self.ctx.req.version,
            "meta_local": self.ctx.meta_local,
            "image_local": self.ctx.image_local,
            "err": self.ctx.err,
